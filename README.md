...
**1. First, the Anaconda/Miniconda program was installed to facilitate the easy installation of bioinformatics tools, specifically MEGAHIT. **
...
library(Biostrings)
library(DECIPHER)
**Read in megahit assembly results for each sample. ‘allcontigs <- c()  Creates an empty object allcontigs, loops from sample 1 to 6, reads each sample’s final.contigs.fa,  and appends all contigs into one big list**
‘allcontigs <- c()’
‘for(i in 1:6){
